...
import autoSuccess from '@/assets/audio/sucesss.mp3';
...
onMounted(() => {
    getUser().then((res) => {
        ...
        socket.on(`updateTransaction-${user.value._id}`, (transaction) => {
            console.log('transaction', transaction);
            store.commit('transaction/setIsDone', true);
            if (transaction.status === 'success') {
                const audio = new Audio(autoSuccess);
                audio.play();
        ...
        socket.on(`updateBalance-${user.value._id}`, (balance) => {
            console.log('balance', balance);
            store.commit('user/updateUser', { balance: balance });
        });
    });
});
    ...
    <a-layout class="layout">
        <a-layout-sider v-model:collapsed="collapsed" :trigger="null" collapsible :breakpoint="'lg'"
            ...
            <a-menu theme="dark" mode="inline" :selectedKeys="selectedKeys">
                ...
                <!-- // giao dịch -->
                <a-menu-item key="2" @click="router.push(`/trade/${roomData[0].time}`)">
                    <TransactionOutlined />
                    <span>Giao dịch</span>
                </a-menu-item>
                <!-- // lịch sử giao dịch -->
                <a-menu-item key="3" @click="router.push('/history')">
                    <HistoryOutlined />
                    <span>Lịch sử giao dịch</span>
                </a-menu-item>
                <!-- // nạp tiền -->
                <a-menu-item key="4" @click="router.push('/deposit')">
                    <MoneyCollectOutlined />
                    <span>Nạp tiền</span>
                </a-menu-item>
                <!-- // rút tiền -->
                <a-menu-item key="5" @click="router.push('/withdraw')">
                    <MoneyCollectOutlined />
                    <span>Rút tiền</span>
                </a-menu-item>
                <!-- // hồ sơ -->
                <a-menu-item key="6" @click="router.push('/profile')">
                    <ProfileOutlined />
                    <span>Hồ sơ</span>
                </a-menu-item>
                <!-- // đăng xuất -->
                <a-menu-item key="7" @click="router.push('/login')">
                    <LoginOutlined />
                    <span>Đăng xuất</span>
                </a-menu-item>
            </a-menu>
